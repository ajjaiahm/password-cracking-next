...
export function AccountSetupPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white p-4">
        <div className="text-center space-y-4">
          ...
          <h1 className="font-mono font-bold text-white">Invalid Setup Link</h1>
          <p className="text-xs text-gray-400 font-mono">This link is invalid or has already been used. Please register again or log in.</p>
          <div className="flex justify-center gap-4 mt-4">
            <button onClick={() => navigate('/login')}
              className="px-4 py-2 bg-zinc-100 text-white text-xs font-mono font-bold rounded hover:bg-white transition-colors">
              Go to Login
            </button>
            <button onClick={() => navigate('/signup')}
              className="px-4 py-2 bg-[#171717] border border-zinc-700 text-gray-300 text-xs font-mono rounded hover:bg-zinc-800 transition-colors">
              Register Again
            </button>
          </div>
        </div>
  ...
  if (done) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-black text-white p-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-[#171717]/90 border border-[#262626] p-10 rounded shadow-2xl relative z-10 backdrop-blur-md text-center space-y-6">
          ...
          <div>
            <h1 className="font-mono font-bold text-xl text-white mb-2">Account Activated</h1>
          ...
          <div className="bg-black border border-[#262626] rounded p-4 text-left space-y-2">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 bg-zinc-100 hover:bg-white text-white font-semibold text-xs uppercase tracking-wider rounded transition-all"
          >
            Go to Login Console <ArrowRight className="w-4 h-4" />
          </button>
          <p className="text-[10px] text-gray-400 font-mono">
            Use email <span className="text-gray-400">{emailFromUrl}</span> to sign in
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-black text-white p-4 font-sans select-none relative overflow-hidden">
      ...
      <div className="w-full max-w-lg bg-[#171717]/95 border border-zinc-700 p-8 rounded-lg shadow-2xl relative z-10 backdrop-blur-xl">

        {/* Header */}
        <div className="flex items-center gap-4 mb-8 pb-6 border-b border-[#262626]">
          <div className="w-12 h-12 rounded-lg border border-blue-600/50 bg-[#171717]/20 flex items-center justify-center shrink-0">
            <ShieldCheck className="w-6 h-6 text-blue-500" />
          </div>
          <div>
            <h1 className="font-mono font-bold text-white text-sm tracking-wider">Secure Account Setup</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Username */}
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-gray-400 font-mono mb-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-black/60 border border-[#262626] focus:border-cyan-700 text-white text-sm pl-10 pr-10 py-3 rounded outline-none transition-colors font-mono placeholder:text-gray-400"
              />
            ...
            <div className="flex items-center justify-between mt-1.5">
              <p className="text-[9px] font-mono text-gray-400">Letters, numbers, _ and - only. Min 3 characters.</p>
                ...
                <div className="grid grid-cols-2 gap-1.5 mt-2">
                  {[
                    ...
                    <div key={label} className={`flex items-center gap-1.5 text-[9px] font-mono px-2 py-1 rounded border transition-colors ${
                      met ? 'border-emerald-800/50 bg-emerald-950/20 text-blue-400' : 'border-[#262626] bg-black/20 text-gray-400'
                    }`}>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={loading}
                placeholder="••••••••"
                className={`w-full bg-black/60 border text-white text-sm pl-10 pr-10 py-3 rounded outline-none transition-colors font-mono placeholder:text-gray-400 ${
                  confirmPassword.length > 0
                    ? passwordsMatch ? 'border-emerald-700 focus:border-emerald-600' : 'border-red-800 focus:border-red-700'
                    : 'border-[#262626] focus:border-cyan-700'
                }`}
                autoComplete="new-password"
              />
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3.5 bg-zinc-100 hover:bg-white text-white font-bold text-xs uppercase tracking-wider rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed"
          >
            {loading ? (
