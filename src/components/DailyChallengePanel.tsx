...
const CHALLENGE_TYPES = [
  ...
  { id: 'wireshark', label: 'Wireshark / PCAP', icon: Radio, desc: 'Network traffic & credential sniffing', color: 'text-blue-400 border-emerald-900/50 bg-emerald-950/20 hover:bg-emerald-950/40' },
];
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2">
                    {isCompleted ? (
                      <span className="inline-flex items-center gap-1 text-[9px] font-mono text-blue-400 bg-emerald-950/30 px-2 py-0.5 rounded border border-emerald-900/30">
                      ...
                      <span className="text-[9px] font-mono text-blue-400">+{CHALLENGE_REWARDS[ct.id]} coins — Generate</span>
                    )}
                  </div>
                </div>
              </button>
        ...
        {selectedType && !dc && !generating[selectedType] && (
          <div className="space-y-5 border-t border-[#262626] pt-8 pb-4 text-center">
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-500 text-white font-mono text-xs uppercase tracking-wider font-bold py-3 px-6 rounded transition-colors"
            >
              Generate Unique Challenge (+{CHALLENGE_REWARDS[selectedType]} coins)
            </button>
          </div>
        )}
          ...
          <div className="space-y-5 border-t border-[#262626] pt-5">
            ...
            <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider">
              <span className={`px-2 py-0.5 rounded border font-bold ${
                ...
                'text-blue-400 border-emerald-900/50 bg-emerald-950/20'
              }`}>{dc.type.toUpperCase()}</span>
              <span className="text-blue-400 font-bold">+{dc.reward} coins</span>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {!dc.completed && dc.hintLevel < dc.hints.length && !firstChallengeFree && (
                  <span className="text-[10px] font-mono text-blue-400">Cost: {hintCost} coins per hint</span>
                )}
                {firstChallengeFree && dc.hintLevel < dc.hints.length && (
                  <span className="text-[10px] font-mono text-blue-400 font-bold">FREE — first challenge</span>
                )}
              </div>
              ...
              {!dc.completed && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-[10px] font-mono text-blue-400 hover:text-amber-400 border border-amber-900/50 bg-amber-950/20 px-3 py-1.5 rounded disabled:opacity-40 transition-colors"
                >
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 rounded border border-[#262626] bg-black/40 text-[11px] font-mono text-gray-400 space-y-3 leading-relaxed">
                {(() => {
                  ...
                  if (dc.type === 'hashcat') {
                    ...
                    return (<><div className="text-gray-300 font-bold text-xs uppercase tracking-wider">hashcat — GPU Password Recovery</div>

                    <div><span className="text-blue-400">Step 1:</span> Save the hash to a file
                    <br/>Copy the hash value above and paste it into a new file called <code className="text-zinc-200 bg-[#171717] px-1 rounded text-[10px]">hash.txt</code>
                    <br/><span className="text-gray-400">Use the terminal on the right: run <code className="text-gray-300 bg-[#171717] px-1 rounded text-[10px]">echo '{hashVal}' &gt; hash.txt</code></span></div>

                    <div><span className="text-blue-400">Step 2:</span> Run hashcat with a wordlist
                    <br/>Run this command in the terminal:
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>hashcat -m {mode} -a 0 hash.txt rockyou.txt</span>
                      <button onClick={() => handleCopyCommand(`hashcat -m ${mode} -a 0 hash.txt rockyou.txt`)} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">• <code className="text-gray-300">-m {mode}</code> = hash type (MD5, SHA1, NTLM, etc.)
                    <br/>• <code className="text-gray-300">-a 0</code> = dictionary attack mode
                    <br/>• <code className="text-gray-300">hash.txt</code> = your target hash file
                    <br/>• <code className="text-gray-300">rockyou.txt</code> = common password wordlist</span></div>

                    <div><span className="text-blue-400">Step 3:</span> Wait for cracking to complete
                    <br/>hashcat will try every word in rockyou.txt against your hash.
                    <br/>Watch the <span className="text-gray-300">Recovered</span> counter — when it reaches <span className="text-blue-400">1/1</span>, the password is found.</div>

                    <div><span className="text-blue-400">Step 4:</span> Show the cracked password
                    <br/>Run this command to display the result:
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>hashcat -m {mode} --show hash.txt</span>
                      <button onClick={() => handleCopyCommand(`hashcat -m ${mode} --show hash.txt`)} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">Output format: <code className="text-gray-300">hash:password</code>
                    <br/>Copy the password part and paste it in the answer field above.</span></div>

                    <div className="pt-1 text-gray-400 text-[10px]">Reference: <a href="https://hashcat.net/wiki/" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">hashcat Wiki</a></div></>);
                  }

                  if (dc.type === 'john') {
                    const format = formatVal || 'raw-md5';
                    return (<><div className="text-gray-300 font-bold text-xs uppercase tracking-wider">John the Ripper — CPU Password Recovery</div>

                    <div><span className="text-blue-400">Step 1:</span> Save the hash to a file
                    <br/>Copy the hash and save it as <code className="text-zinc-200 bg-[#171717] px-1 rounded text-[10px]">hash.txt</code>
                    <br/><span className="text-gray-400">Use: <code className="text-gray-300 bg-[#171717] px-1 rounded text-[10px]">echo '{hashVal}' &gt; hash.txt</code></span></div>

                    <div><span className="text-blue-400">Step 2:</span> Run John with a wordlist
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>john --wordlist=rockyou.txt hash.txt</span>
                      <button onClick={() => handleCopyCommand('john --wordlist=rockyou.txt hash.txt')} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">• <code className="text-gray-300">--wordlist=rockyou.txt</code> = dictionary file
                    <br/>• John auto-detects the hash format</span></div>

                    <div><span className="text-blue-400">Step 3:</span> Show cracked passwords
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>john --show hash.txt</span>
                      <button onClick={() => handleCopyCommand('john --show hash.txt')} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">If the format is wrong, try: <code className="text-gray-300">john --format={format} --wordlist=rockyou.txt hash.txt</code></span></div>

                    <div className="pt-1 text-gray-400 text-[10px]">Reference: <a href="https://www.openwall.com/john/doc/" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">John the Ripper docs</a></div></>);
                  }

                  if (dc.type === 'hydra') {
                    const host = hostVal || '192.168.1.100';
                    const user = userVal || 'admin';
                    return (<><div className="text-gray-300 font-bold text-xs uppercase tracking-wider">Hydra — Online Network Brute-Forcing</div>

                    <div><span className="text-blue-400">Step 1:</span> Identify the target service
                    <br/>Target host: <code className="text-zinc-200 bg-[#171717] px-1 rounded text-[10px]">{host}</code>
                    <br/>Target username: <code className="text-zinc-200 bg-[#171717] px-1 rounded text-[10px]">{user}</code>
                    <br/>Common services: ssh (port 22), ftp (21), rdp (3389), http-post (80)</div>

                    <div><span className="text-blue-400">Step 2:</span> Run Hydra
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>hydra -l {user} -P rockyou.txt ssh://{host}</span>
                      <button onClick={() => handleCopyCommand(`hydra -l ${user} -P rockyou.txt ssh://${host}`)} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">• <code className="text-gray-300">-l {user}</code> = single username to try
                    <br/>• <code className="text-gray-300">-P rockyou.txt</code> = password wordlist
                    <br/>• <code className="text-gray-300">ssh://{host}</code> = target service &amp; address
                    <br/>Try different services: <code className="text-gray-300">ftp://{host}</code>, <code className="text-gray-300">rdp://{host}</code></span></div>

                    <div><span className="text-blue-400">Step 3:</span> Read the results
                    <br/>Hydra will output lines like:
                    <br/><code className="text-gray-300 text-[10px]">[22][ssh] host: {host} login: {user} password: FOUND_PASSWORD</code>
                    <br/>The password after <code className="text-gray-300">password:</code> is your answer.</div>

                    <div className="pt-1 text-gray-400 text-[10px]">Reference: <a href="https://github.com/vanhauser-thc/thc-hydra" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Hydra GitHub</a></div></>);
                  }
                  ...
                  if (dc.type === 'wireshark') {
                    ...
                    return (<><div className="text-gray-300 font-bold text-xs uppercase tracking-wider">Wireshark / tshark — Network Traffic Analysis</div>

                    <div><span className="text-blue-400">Step 1:</span> Inspect the PCAP file
                    <br/>Run tshark to list all packets:
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>tshark -r {targetVal}</span>
                      <button onClick={() => handleCopyCommand(`tshark -r ${targetVal}`)} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">This shows all packets. Look for protocols like HTTP, FTP, or Telnet (plaintext credentials).</span></div>

                    <div><span className="text-blue-400">Step 2:</span> Filter for login traffic
                    <br/>Look for POST requests or login pages:
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      <span>tshark -r {targetVal} -Y "http.request.method == POST"</span>
                      <button onClick={() => handleCopyCommand(`tshark -r ${targetVal} -Y "http.request.method == POST"`)} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">Also try: <code className="text-gray-300">tshark -r {targetVal} -Y "ftp"</code> or <code className="text-gray-300">-Y "telnet"</code></span></div>

                    <div><span className="text-blue-400">Step 3:</span> Extract the credential data
                    ...
                    <div className="bg-[#171717] border border-[#262626] rounded p-2 mt-1 text-[10px] text-zinc-200 flex items-center justify-between">
                      ...
                      <button onClick={() => handleCopyCommand(`tshark -r ${targetVal} -Y "http.request.method == POST" -T fields -e http.file_data`)} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0 ml-2"><Copy className="w-3 h-3" /></button>
                    </div>
                    <span className="text-gray-400">Look for patterns like <code className="text-gray-300">username=admin&amp;password=secret123</code></span></div>

                    <div><span className="text-blue-400">Step 4:</span> Submit the found credential
                    <br/>The password is the value after <code className="text-gray-300">password=</code> or equivalent field.</div>

                    <div className="pt-1 text-gray-400 text-[10px]">Reference: <a href="https://www.wireshark.org/docs/" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Wireshark docs</a></div></>);
                  }
                  return null;
                })()}
              </div>
            ...
            {feedback[selectedType!] && (
              <div className={`p-3 rounded border text-xs font-mono flex items-start gap-2 ${
                feedback[selectedType!]!.ok
                  ? 'bg-emerald-950/20 border-emerald-900/40 text-blue-400'
